...
- Nullable: NO (in theory) — but check for duplicates
- Sample value: e481f51cbdc54678b7cc49136f2d6af7
- Issue found: 80 duplicates
- Transform needed: Deduplicate keeping highest lifecycle status
...
- Transform needed: LOWER() + strip whitespace + validate against enum list
...
- Transform needed: Detect format → parse → convert to UTC

### order_approved_at
- Type: Timestamp
- Nullable: YES — [153]
- Note: NULL for cancelled orders is EXPECTED and VALID — not a data error
- Transform needed: Parse to UTC, preserve nulls
...
- Transform needed: Parse to UTC; quarantine rows where delivered < purchased

### order_estimated_delivery_date
- Type: Date (no time component)
- Nullable: 0
- Use: Key input for computing delivery_delay_days KPI
- Transform needed: Parse date; compute delivery_delay_days = delivered - estimated
